...
- `HttpOpenSync.js` — Web App `doPost` / `doOptions`: 본문 `action=ping|syncOpenFull` → `dbSyncOpenAll()`. **`plannerMatch` / `plannerBootstrap` / `initPlannerMasterSheets`** (`dbPlanner.js`)는 `POST` + `Content-Type: text/plain` JSON 본문(임웹 `fetch` — [docs/PLANNER_DB_ARCHITECTURE.md](../docs/PLANNER_DB_ARCHITECTURE.md)). 브라우저·CORS·JSONP·시트 `getRange` 규칙 → [docs/GAS_WEBAPP_SHEETS.md](../docs/GAS_WEBAPP_SHEETS.md) · [docs/BACKEND_API.md](../docs/BACKEND_API.md) §2
