...
- **Bearer 시크릿** — 예: `GET /site-info` → `Authorization: Bearer <앱 시크릿>` (문서 표기 `YOUR_SECRET_TOKEN`; 값은 Script Properties만).
- **Bearer 액세스 토큰** — 예: `GET /site-info/unit/{unitCode}` → `Authorization: Bearer <액세스 토큰>` (문서 표기 `YOUR_ACCESS_TOKEN`; 보통 `POST /v2/auth` 등 발급분). `/v2/*` 호출과 동일 계열로 맞출 것.
...
#### 유닛(몰) 정보 — `GET /site-info/unit/{unitCode}`

경로 파라미터 `unitCode`는 유닛 식별자 (문서 예: `u20210810611211bd954ec`). `GET /site-info` 응답의 `unitList` 등과 연계해 확정.

| Method | URL (전체) | Headers | Request body |
|--------|------------|---------|--------------|
| GET | `https://openapi.imweb.me/site-info/unit/{unitCode}` | `Authorization: Bearer <YOUR_ACCESS_TOKEN>` | — |

성공 시 **HTTP 200**, 바디 **패턴 B** (`statusCode` + `data`).
...
```json
{
  ...
  "data": {
    ...
    "presidentName": "홍길동",
    "phone": "01012345678",
    "email": "example@imweb.me",
    "address1": "서울 강남구 테헤란로 501 (삼성동, 브이플렉스)",
    "address2": "14층",
    "companyRegistrationNo": "123-45-67890",
    "primaryDomain": "example.imweb.me"
  }
}
```
...
| `unitCode` | string | 유닛 코드 |
| `langCode` | string | 언어 (예: `KR`) |
| `currency` | string | 통화 (예: `KRW`) |
| `currencyFormat` | string | 통화 표시 형식 (문서 예: `currency`) |
| `name` | string | 사이트(유닛) 이름 |
| `isDefault` | string | 기본 유닛 여부 (문서 예: `Y` / `N`) |
| `companyName` | string | 상호 |
| `presidentName` | string | 대표자명 |
| `phone` | string | 전화번호 |
| `email` | string | 이메일 |
| `address1` | string | 주소 1 |
| `address2` | string | 주소 2 |
| `companyRegistrationNo` | string | 사업자등록번호 |
| `primaryDomain` | string | 기본 도메인 (예: `*.imweb.me`) |

*cURL*

```bash
curl https://openapi.imweb.me/site-info/unit/u20210810611211bd954ec \
  --header 'Authorization: Bearer YOUR_ACCESS_TOKEN'
```

#### 연동 완료 처리 — `PATCH /site-info/integration-complete`

앱 연동이 **`연동중`**일 때, 나머지 Open API가 막힐 수 있음 → 이 호출로 완료 처리(SPEC·process의 부트스트랩).

| Method | URL (전체) | Headers | Request body |
|--------|------------|---------|--------------|
| PATCH | `https://openapi.imweb.me/site-info/integration-complete` | `Authorization: Bearer <YOUR_ACCESS_TOKEN>`, `Content-Type: application/json` | JSON 객체 |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `configData` | object | 문서 예시는 빈 객체. 공식 스펙에 추가 키가 있으면 실측 후 보강 |
...
| 2026-04-25 | `GET /site-info/unit/{unitCode}` — Bearer **액세스** 토큰, `data`에 몰·사업자·도메인 필드 (문서 샘플 반영) |
| 2026-04-25 | `PATCH /site-info/integration-complete` — JSON 바디 `configData`, 응답 `data: true` (연동 완료 처리) |
